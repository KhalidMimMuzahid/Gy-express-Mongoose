...
const getDashboardStatsController = async (req, res) => {
  try {
    ...
    const team = await Level.findOne({ userId: req.auth });
    const totalTeam = team?.level?.length;
    const totalDirectTeam = team?.level?.filter((l) => l.level === "1").length;
    const walletFind = await Wallet.findOne({ userId: req.auth });
    // ROI Income
    const roiIncome = await PackageRoi.findOne({ userId: req.auth });
    // ROI Table
    const roiHistory = roiIncome?.history;
    // Topup history
    const topupHistory = await PackageBuyInfo.find({ userId: req.auth });
    const data = {
      totalTeam,
      totalDirectTeam,
